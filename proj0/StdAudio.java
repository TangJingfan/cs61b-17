...
public final class StdAudio {
  ...
  private static int bufferSize =
      0; // number of samples currently in internal buffer
  ...
  private static void init() {
    try {
      ...
      // Endian
      AudioFormat format =
          new AudioFormat((float)SAMPLE_RATE, BITS_PER_SAMPLE, 1, true, false);
      ...
      // choice is arbitrary it gets divided because we can't expect the
      // buffered data to line up exactly with when the sound card decides to
      // push out its samples.
  ...
  /**
   * Read audio samples from a file (in .wav or .au format) and return them as a
   * double array with values between -1.0 and +1.0.
   */
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i <= StdAudio.SAMPLE_RATE; i++) {
      StdAudio.play(0.5 *
                    Math.sin(2 * Math.PI * freq * i / StdAudio.SAMPLE_RATE));
    }
