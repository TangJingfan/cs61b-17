 ...
 *  (between 0 and 1 and by default) or the points and lines will not be
 * visible. <p> <b>Squares, circles, rectangles, and ellipses.</b> You can draw
 * squares, circles, rectangles, and ellipses using the following methods: <ul>
 *  <li> {@link #circle(double x, double y, double radius)}
 *  <li> {@link #ellipse(double x, double y, double semiMajorAxis, double
 * semiMinorAxis)} <li> {@link #square(double x, double y, double radius)} <li>
 ...
 * ellipses, but may be unexpected to the uninitiated. <p> The methods above
 * trace outlines of the given shapes. The following methods draw filled
 * versions: <ul> <li> {@link #filledCircle(double x, double y, double radius)}
 *  <li> {@link #filledEllipse(double x, double y, double semiMajorAxis, double
 * semiMinorAxis)} <li> {@link #filledSquare(double x, double y, double radius)}
 *  <li> {@link #filledRectangle(double x, double y, double halfWidth, double
 * halfHeight)}
 *  </ul>
 *  <p>
 *  <b>Circular arcs.</b>
 ...
 *  <ul>
 *  <li> {@link #arc(double x, double y, double radius, double angle1, double
 * angle2)}
 *  </ul>
 *  <p>
 ...
 *  and represented in degrees. For example, {@code StdDraw.arc(0.0, 0.0, 1.0,
 * 0, 90)} draws the arc of the unit circle from 3 o'clock (0 degrees) to 12
 * o'clock (90 degrees). <p> <b>Polygons.</b> You can draw polygons with the
 * following methods: <ul> <li> {@link #polygon(double[] x, double[] y)} <li>
 ...
 *  you can use this method with one of these predefined colors in standard
 * drawing:
 *  {@link #BLACK}, {@link #BLUE}, {@link #CYAN}, {@link #DARK_GRAY}, {@link
 * #GRAY},
 ...
 * <em>y</em>). <p> The default font is a Sans Serif font with point size 16.
 ...
 *  <li> {@link #picture(double x, double y, String filename, double
 * scaledWidth, double scaledHeight)} <li> {@link #picture(double x, double y,
 * String filename, double scaledWidth, double scaledHeight, double degrees)}
 *  </ul>
 *  <p>
 *  These methods draw the specified image, centered at (<em>x</em>,
 * <em>y</em>). The supported image formats are JPEG, PNG, and GIF. The image
 * will display at its native size, independent of the coordinate system.
 *  Optionally, you can rotate the image a specified number of degrees
 * counterclockwise or rescale it to fit snugly inside a width-by-height
 * bounding box. <p> <b>Saving to a file.</b> You save your image to a file
 * using the <em>File → Save</em> menu option. You can also save a file
 * programatically using the following method: <ul> <li> {@link #save(String
 * filename)}
 *  </ul>
 *  <p>
 ...
 * solely of geometric shapes and JPEG for drawings that contains pictures. <p>
 ...
 *  <p>
 *  When double buffering is enabled by calling {@link
 * #enableDoubleBuffering()}, all drawing takes place on the <em>offscreen
 * canvas</em>. The offscreen canvas is not displayed. Only when you call
 ...
 * You can think of double buffering as collecting all of the lines, points,
 * shapes, and text that you tell it to draw, and then drawing them all
 ...
 *  It is much less powerful than most user interface libraries provide, but
 * also much simpler. You can use the following methods to intercept mouse
 * events: <ul> <li> {@link #isMousePressed()} <li> {@link #mouseX()} <li>
 * {@link #mouseY()}
 *  </ul>
 *  <p>
 *  The first method tells you whether a mouse button is currently being
 * pressed. The last two methods tells you the <em>x</em>- and
 * <em>y</em>-coordinates of the mouse's current position, using the same
 * coordinate system as the canvas (the unit square, by default). You should use
 * these methods in an animation loop that waits a short while before trying to
 * poll the mouse for its current state. You can use the following methods to
 * intercept keyboard events: <ul> <li> {@link #hasNextKeyTyped()} <li> {@link
 * #nextKeyTyped()} <li> {@link #isKeyPressed(int keycode)}
 *  </ul>
 *  <p>
 ...
 * key is currently being pressed. <p> <b>Accessing control parameters.</b> You
 * can use the following methods to access the current pen color, pen radius,
 *  and font:
 *  <ul>
 *  <li> {@link #getPenColor()}
 ...
 * <em>y</em>-coordinate that is NaN will behave as if it is outside the canvas,
 * and will not be visible. <li> Due to floating-point issues, an object drawn
 * with an <em>x</em>- or <em>y</em>-coordinate that is way outside the canvas
 * (such as the line segment from (0.5, –&infin;) to (0.5, &infin;) may not be
 * visible even in the part of the canvas where it should be.
 *  </ul>
 *  <p>
 ...
 *  <ul>
 *  <li> The {@code picture()} methods may not draw the portion of the image
 * that is inside the canvas if the center point (<em>x</em>, <em>y</em>) is
 * outside the canvas. This bug appears only on some systems. <li> Some methods
 * may not draw the portion of the geometric object that is inside the canvas if
 * the <em>x</em>- or <em>y</em>-coordinates are infinite. This bug appears only
 * on some systems.
 *  </ul>
 *  <p>
 *  <b>Reference.</b>
...
public final class StdDraw
   ...
   * {@code StdDraw.LIGHT_GRAY}, {@code StdDraw.MAGENTA}, {@code
   * StdDraw.ORANGE},
   ...
   * To achieve the same effect, set the pen radius to 0 and call {@code
   * point()}.
   *
  ...
  public static void arc(double x, double y, double radius, double angle1,
    ...
    if (ws <= 1 && hs <= 1)
      ...
      offscreen.draw(new Arc2D.Double(xs - ws / 2, ys - hs / 2, ws, hs, angle1,
   ...
   * @throws IllegalArgumentException if either {@code halfWidth} or {@code
  ...
  /*
      private static BufferedImage getImage(String filename) {
          ...
          throw new IllegalArgumentException("image " + filename + " not
     found");
      }
  */
  /**
   ...
   * penalty for redrawing the same image multiple times (e.g., in an
   * animation). However, if you change the picture file after drawing it,
  ...
  public static void picture(double x, double y, String filename) {
    ...
    // BufferedImage int hs = image.getHeight();
   ...
   */
  public static void enableDoubleBuffering() { defer = true; }
   ...
   * This is the default.
   */
  public static void disableDoubleBuffering() { defer = false; }
  ...
  public static void save(String filename) {
    ...
    // http://archives.java.sun.com/cgi-bin/wa?A2=ind0404&L=java2d-interest&D=0&P=2727
      ...
      DirectColorModel newCM =
          new DirectColorModel(cm.getPixelSize(), cm.getRedMask(),
  ...
  public void actionPerformed(ActionEvent e) {
    FileDialog chooser = new FileDialog(
        StdDraw.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
  ...
  public static boolean isMousePressed() {
    synchronized (mouseLock) { return isMousePressed; }
  }
  ...
  public static double mouseX() {
    synchronized (mouseLock) { return mouseX; }
  }
  ...
  public static double mouseY() {
    synchronized (mouseLock) { return mouseY; }
  }
  ...
  public void mouseReleased(MouseEvent e) {
    synchronized (mouseLock) { isMousePressed = false; }
  }
  ...
  public static boolean hasNextKeyTyped() {
    synchronized (keyLock) { return !keysTyped.isEmpty(); }
  }

  /**
   * Returns the next key that was typed by the user (that your program has not
   * already processed). This method should be preceded by a call to {@link
   * #hasNextKeyTyped()} to ensure that there is a next key to process. This
   * method returns a Unicode character corresponding to the key typed (such as
   * {@code 'a'} or {@code 'A'}). It cannot identify action keys (such as F1 and
   * arrow keys) or modifier keys (such as control).
   *
  ...
  public static boolean isKeyPressed(int keycode) {
    synchronized (keyLock) { return keysDown.contains(keycode); }
  }
  ...
  public void keyTyped(KeyEvent e) {
    synchronized (keyLock) { keysTyped.addFirst(e.getKeyChar()); }
  }
  ...
  public void keyPressed(KeyEvent e) {
    synchronized (keyLock) { keysDown.add(e.getKeyCode()); }
  }
  ...
  public void keyReleased(KeyEvent e) {
    synchronized (keyLock) { keysDown.remove(e.getKeyCode()); }
  }
