...
public class TestReadPlanets {
  ...
  private static boolean doubleEquals(double actual, double expected,
                                      double eps) {
  ...
  private static String checkReadPlanets() {
    ...
    if (actualOutput.length != 5) {
      return "FAIL: readPlanets().length: Expected 5 and you gave " +
          actualOutput.length;
    }
    ...
    for (Planet p : actualOutput) {
      if ("earth.gif".equals(p.imgFileName)) {
        ...
        if (!doubleEquals(p.xxPos, 1.4960e+11, 0.01)) {
          System.out.println(
              "Advice: Your Earth doesn't have the right xxPos!");
        ...
        if (!doubleEquals(p.yyPos, 0, 0.01)) {
          System.out.println(
              "Advice: Your Mercury doesn't have the right yyPos!");
          randomChecksOkay = false;
        }
